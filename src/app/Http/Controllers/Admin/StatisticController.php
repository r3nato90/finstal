...
class StatisticController extends Controller
{
    ...
    {
        $setTitle = __('Transaction Logs Management');
        ...
        $allowedSortFields = ['trx', 'amount', 'type', 'wallet_type', 'source', 'created_at'];
        ...
        $transactionLogs = $query->paginate($perPage)->appends(request()->all());
    ...
    private function getTransactionStats(): array
    {
        return [
            ...
            'totalCredit' => Transaction::where('type', \App\Enums\Transaction\Type::PLUS->value)->sum('amount'),
            'totalDebit' => Transaction::where('type', \App\Enums\Transaction\Type::MINUS->value)->sum('amount'),
            'totalCharge' => Transaction::sum('charge'),
