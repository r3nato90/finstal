...
class InvestmentService
{
    ...
    public function prepParams(int|string|float $amount, InvestmentPlan $investmentPlan): array
    {
        ...
        return [
            ...
            'profit_time' => $this->calculateProfitTime($investmentPlan->timeTable->time ?? 1),
    ...
    private function calculateProfitTime(int $hours): string
    {
        $holidaySettings = (array) Setting::get('holiday_setting', ["sunday","monday","tuesday","wednesday","thursday"]);

        Log::info("Holiday settings: " . json_encode($holidaySettings));
        Log::info("Adding hours: " . $hours);

        $targetTime = now()->addHours($hours);
        Log::info("Initial target time: " . $targetTime->toDateTimeString());

        $maxAttempts = 30;
        $attempts = 0;

        while ($attempts < $maxAttempts) {
            $checkDate = $targetTime->format('Y-m-d');
            $dayOfWeek = strtolower($targetTime->format('l'));

            Log::info("Checking date: {$checkDate}, day: {$dayOfWeek}");

            $isHoliday = Holiday::where('date', $checkDate)->exists();
            $isNonWorkingDay = in_array($dayOfWeek, $holidaySettings);

            Log::info("Is holiday: " . ($isHoliday ? 'yes' : 'no'));
            Log::info("Is non-working day: " . ($isNonWorkingDay ? 'yes' : 'no'));

            if (!$isNonWorkingDay && !$isHoliday) {
                Log::info("Found valid date: " . $targetTime->toDateTimeString());
                return $targetTime->toDateTimeString();
            }

            $targetTime->addDay();
            $attempts++;
            Log::info("Moving to next day, attempt: " . $attempts);
        }

        $fallback = now()->addHours($hours)->toDateTimeString();
        Log::warning("Using fallback time: " . $fallback);
        return $fallback;
    }
